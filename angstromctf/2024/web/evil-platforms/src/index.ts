...
const PUBLIC_KEYS = {
	'1243802014991913034': '0db7dc700b1a30928ee52a198ae6cb32a96440128cedc47955a787b2ae6ad391',
};
		...
		if (request.method === 'POST' && url.pathname === '/interact') {
			...
			const valid = await crypto.subtle.verify(
				...
				await crypto.subtle.importKey(
					'raw',
					// @ts-expect-error
					hexDecode(PUBLIC_KEYS[contents.application_id ?? '1243802014991913034']),
			...
			if (contents.type === 1) {
				...
				if (user?.id === '302968847353249813') {
					return new Response(JSON.stringify({ type: 4, data: { content: env.flag, flags: 64 } }), {
...
// `npx wrangler tail --format pretty | Out-File -FilePath .\logs.txt -Encoding utf8`, then copy that here
