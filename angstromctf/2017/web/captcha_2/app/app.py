...
def genImage(location):
    ...
    sample = list(range(0,len(numbers)))
    random.shuffle(sample)
    choices = sample[:5]
    return (numbers,choices)


numbers,choices = genImage('captcha.png')
...
def main():
    global numbers,choices,filename
    ...
    if form.validate_on_submit():
        if form.question1.data == int(''.join([str(numbers[choice]) for choice in choices])):
            numbers,choices = genImage(filename)
            if form.pin.data == 753:
                return render_template('index.html',
                                       flag=flag,
                                       words=', '.join([informationPrompt[choice] for choice in choices[:-1]])+', and '+informationPrompt[choices[-1]],
                                       form=form,
